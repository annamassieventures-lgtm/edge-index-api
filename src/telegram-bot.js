...
This is the first thing the client reads. Format it as a bold, scannable reference page. List exactly 3-5 Golden Windows — the periods where this person should move with maximum conviction. For each window give: the month range only (e.g. "Late April — Mid June 2026"), a one-line description of why it's high-leverage for THIS person specifically, and one action instruction (e.g. "Deploy capital. Initiate. Scale. Expand."). Then list 2-3 PROTECTION PERIODS with the same format.

CRITICAL INSTRUCTION — The Brief names the windows by month range only. It does NOT reveal the specific week or day within each window when signals peak. That precision is intentionally withheld — it requires real-time monitoring to catch the exact moment a window opens. At the bottom of this page, include exactly this line in italics: "The windows above show when conditions favour decisive action. To know the exact week and day your window opens — and when to stand down — that requires live signal tracking. See Section 15."

This creates the right tension: the client has enormous value from knowing their windows, and a clear reason why monitoring matters.
